...
| **压缩过程在对话区可见** | 默认开启；`notice: false` 关闭 | 订阅 `session/event`，`compaction/start` 一落地就往会话尾追加一条插件来源的 `user/message`，客户端渲染成「上下文注入 · dsh-compact-agents」折叠行：压缩中显示 *正在压缩上下文…（当前 213,400 tokens）· 触发线 ×0.2*，结束时显示 *上下文压缩完成：约 213,400 → 49,800 tokens，已遮蔽 37 个历史节点*。`compaction/start` 是在摘要模型调用**之前**写的，这段提示正好盖住原本什么都看不见的等待。**提示里会报出本会话实际生效的触发线**；如果 preset 文件已经改过而本会话还是旧代际，折叠行与正文会直接点明两个值与"新开一条对话才生效" |
...
这也就是"我明明把阈值改成 0.5 了，怎么还在按 200K 压"的答案：**不是要重启，是要新开一条对话**（重启对已在跑的会话同样无效）。为了让这件事当场可见，压缩提示里会报出**本会话实际生效的触发线**，并在 preset 已经改过时直接写明两个值与"新开一条对话才会用上新值"。
...
```
▸ 上下文注入 · dsh-compact-agents · 正在压缩上下文…（当前 213,400 tokens） · 触发线 ×0.2
...
折叠行末尾那个 `触发线 ×0.2` 是**本会话这一代际实际生效的值**；若 preset 文件已经被改成别的值，
展开后还会多一行 —— 这就是"我改了 preset 怎么没生效"的现场答案：

```
⚠️ preset 文件里现在是 ×0.5，本会话仍按 ×0.2：压缩参数在会话建立时读取，新开一条对话才会用上新值。
```
...
- **v0.6.1** — 压缩提示报出**本会话实际生效的触发线**（`· 触发线 ×0.2`），并在 preset 文件已经改过、而本会话仍是旧代际时直接写明两个值与"新开一条对话才会用上新值"。起因是一个必然会撞上的困惑：preset 里的压缩参数在**会话建立时**读进 `compaction-basic` 并深冻结，之后改文件只对新建会话生效（`agent-preset/locked`），而旧提示只报 token 数，看不出用的是哪一档阈值。同时把设置卡片里那句会误导人的 hint 改成显式说明"默认 0.35"——它此前写成"（0.35 = 350K tokens）"，紧挨着一个用户自己设的 0.5，读起来像当前值。
