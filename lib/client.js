/**
 * dsh-compact-agents 的浏览器 half：压缩参数的表单，挂到 DSH 的三个设置宿主上。
 *
 * ## 三处注册，一份表单
 *
 * 上游 `90af3110b7 feat(web): host plugin configuration on the Plugins page` 把插件配置
 * 从「设置 → 插件 → 可配置」搬到了侧栏新的**插件页**（`ui-plugin-manager`）。老槽位
 * `settings.plugin.item` 在新版里**已经没有任何渲染方**（官方把 ConfigurablePluginsTab
 * 删了），只注册它 = 卡片凭空消失、还没有任何报错。所以这里三处都注册，谁有宿主谁渲染：
 *
 *   1. `settings.section`（**list** 槽位，主要入口）：在设置侧栏里自成一页，与「通用设置 /
 *      模型 / 内置插件 / Agent 预设」同级（第三方插件也用它，如「费用」「插件市场」）。
 *      形状是 `id`（认领位置）+ `order`（排位）+ `label`（侧栏那一行文字），组件拿到的是
 *      空的 owner props + 注入面，**标题与引言得自己画**；
 *   2. `plugins.bundle.config`（keyed 槽位，键 = 宿主 bundle 的**包名**）：新版插件页里本包
 *      详情页的表单（`renderSlot('plugins.bundle.config', { view: 'page' }, { entryKey: pkg.name })`）。
 *      页自己画标题、图标与面包屑，所以组件只许渲染**表单主体**，`view: 'summary'` 时给一行摘要；
 *   3. `settings.plugin.item`（keyed 槽位，键 = settings 命名空间 `compact-agents`）：老设置页
 *      （≤ 0.1.5）「插件 → 可配置」里的折叠卡片，头部、箭头、收起全由组件自己画。
 *
 * 为什么第 1 处才是主入口：插件页那条路要用户「进插件页 → 找到本包 → 进详情」才能改参数，
 * 而且官方随时可能再挪一次；`settings.section` 是官方给插件的一等公民位置，老版新版都在。
 *
 * `ctx.slots.inject(name, cb)` 在槽位**尚未声明**时只是安静等待、声明后才跑 cb，因此
 * 对三个槽位都调用它是安全的（老版本等不到新槽，新版等不到旧槽），无需版本判断。
 * 每个 cb 里的 `register` 仍各自 try/catch：同一 key/id 重复注册会抛，一处注册不上也不该
 * 影响另外两处。
 *
 ...
 *   3. 按 settings 命名空间读写：三处注册都指向同一份表单与同一个 `settingsScope` 绑定，
 *      所以参数与「已覆盖 / 重置」的语义各处完全一致。
 *   4. 任何一处都不抛异常：命名空间未就绪（`status !== 'ready'`）时只渲染一句
 *      「设置尚未就绪」。设置分区与老卡片都可能和别人的组件渲染在同一棵树里，一处抛错会毁掉整页。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 宿主 half 注册的 settings 命名空间；老设置页的卡片按它配对，两端必须逐字一致。 */
    const NAMESPACE = 'compact-agents';

    /**
     * 本插件的包名 —— 新版插件页的 `plugins.bundle.config` 用**包名**当注册键，
     * 必须与 package.json 的 name、以及 profile `dependencies` 里的键逐字一致
     * （新页拿 `pkg.name` 去查表单，差一个字符就是"表单不见了"且毫无报错）。
     */
    const PACKAGE_NAME = 'dsh-compact-agents';

    /** 卡片标题。 */
    const TITLE = '压缩与自动续写';

    /** 卡片总说明：与官方卡片一样，标题下压一行描述，正文里不再重复。 */
    const DESCRIPTION = '强制压缩的触发阈值、保留比例与输出预算，回合结束预压，以及自动续写次数。';
    ...
    /**
     * 卡片编辑的六个字段 —— 冻结的接口，字段名/类型/范围由宿主 half 的 settings 段决定。
     * kind 决定保存前的校验（'boolean' 只收 true/false，'int' 收整数，'number' 收数值），
     * tier 决定分组：live 三项宿主每次动作都读；hot 两项由本插件热同步进运行中的
    ...
    const FIELDS = [
      ...
      {
        field: 'preemptiveRatio',
        kind: 'number',
        min: 0,
        max: 1,
        tier: 'live',
        label: '回合结束预压比例',
        hint: '一轮结束时占用已达触发线的这个比例就先压一次，把摘要耗时挪到回答之后的空档里；0 表示关闭，默认 0.9（建议 0.8–0.95）。',
      },
      {
    ...
    const CSS = [
      ...
      // 插件页（新宿主）里的表单主体：页自己已经给了 section 与标题，这里不再套卡片外壳，
      // 只留内边距与字段分组 —— 观感与设置页那张卡展开后的正文一致。
      '.dsh-ca-page{padding:2px 0 4px}',
      // 插件页的列表项问一行摘要时用（list 槽位才要，bundle 槽位只问 page，这里是兜底）。
      '.dsh-ca-summary{margin:0;font-size:13px;line-height:1.5;color:var(--dsw-alias-label-secondary,#646a73)}',
      // 自成一页的设置分区（槽位 `settings.section`）：壳只渲染这一列，标题与引言由分区自己画，
      // 尺寸照抄官方 PluginsSettingsSection.module.css 的 .section/.heading/.intro。
      '.dsh-ca-section{display:flex;flex-direction:column;gap:12px;max-width:760px;color:var(--dsw-alias-label-primary,#1f2329)}',
      '.dsh-ca-heading{margin:0;font-size:18px;font-weight:600}',
      '.dsh-ca-intro{margin:0;font-size:13px;line-height:1.5;color:var(--dsw-alias-label-tertiary,#a3a8b0)}',
    ...
    /**
     * 表单主体：字段按生效时机分组 + 末尾动作条。
     *
     * 两个宿主共用这唯一一份正文 —— 老设置页把它塞进折叠卡片的 `.dsh-ca-body`，
     * 新插件页把它直接放进自己的 section（标题、图标、面包屑都由页画）。抽出来是为了让
     * "两处必须长得一样"在代码里只有一处定义，否则改一处忘一处就是又一次"文字与行为不符"。
     * @param state - 卡片状态快照（fields / writable / dirty / failed / saving / invalid）。
     * @param face - 注入面（表单动作）。
     * @returns 子节点数组。
     */
    function formBody(state, face) {
    ...
    /**
     * 两行状态里挑最关键的两个比例，拼成一行摘要 —— 只有新插件页的**列表项**槽会问一行字
     * （`plugins.bundle.config` 只问 `view: 'page'`，这里是兜底，不参与主路径）。
     * @param state - 卡片状态快照。
     * @returns 摘要文案。
     */
    function summaryText(state) {
      if (state.available !== true) return statusText(state.status);
      const fields = state.fields ?? {};
      const parts = ['thresholdRatio', 'retainRatio']
        .map(field => FIELD_BY_NAME.get(field))
        .filter(spec => spec !== undefined)
        .map(spec => `${spec.label} ${fields[spec.field]?.text ?? '—'}`);
      return parts.length > 0 ? parts.join(' · ') : TITLE;
    }

    /**
     * 新插件页里的表单（槽位 `plugins.bundle.config`，`view: 'page'`）。
     *
     * 与 `CompactAgentsCard` 的区别只有外壳：这里**不许**再画卡片头与折叠箭头 ——
     * 插件页自己已经给了标题、图标与面包屑，套第二层壳就会长出两个标题。
     * @param props - 渲染器绑定的注入面 + `view`（'page' | 'summary'）。
     * @returns 表单元素。
     */
    function CompactAgentsPanel(props) {
      const face = props ?? {};
      ...
      if (state.available !== true) {
        return h('p', { className: 'dsh-ca-status' }, statusText(state.status));
      }
      if (face.view === 'summary') {
        return h('p', { className: 'dsh-ca-summary' }, summaryText(state));
      }
      return h('div', { className: 'dsh-ca-page' }, formBody(state, face));
    }

    /**
     * 自成一页的设置分区（槽位 `settings.section`，`id: compact-agents`）。
     *
     * 这是**和「内置插件」同级**的那个入口：设置侧栏里多一行「压缩与自动续写」，点进去就是这张
     * 表单。用它与用插件页的区别只是"谁画标题" —— 设置壳只渲染这一列内容（`SettingsRoot` 的
     * `css.options` 里 `renderSlot('settings.section', { close }, { only })`），标题与引言得自己画；
     * 插件页则反过来，页画标题、我们只给正文。
     *
     * 为什么不挂在插件页里就够了：上游 0.1.6 把插件配置搬去插件页之后，用户要在**插件页 →
     * 找到本包 → 进详情**才能改参数；而"设置里多一个自己的分区"是官方给插件留的一等公民位置
     * （通用设置 / 模型 / 内置插件 / Agent 预设 与第三方插件都用它），与宿主版本无关 ——
     * 老版新版都在，不必赌槽位还在不在。
     * @param props - 渲染器绑定的注入面（owner props 里的 `close` 这里用不上）。
     * @returns 分区元素。
     */
    function CompactAgentsSection(props) {
      const face = props ?? {};
      let state = PENDING_STATE;
      try {
        if (typeof face.useCompactAgentsCard === 'function') {
      ...
      if (state.available !== true) {
        return h('div', { className: 'dsh-ca-section' }, [
          h('h2', { className: 'dsh-ca-heading', key: 'title' }, TITLE),
          h('p', { className: 'dsh-ca-status', key: 'status' }, statusText(state.status)),
        ]);
      }
      return h('div', { className: 'dsh-ca-section' }, [
        h('h2', { className: 'dsh-ca-heading', key: 'title' }, TITLE),
        h('p', { className: 'dsh-ca-intro', key: 'intro' }, DESCRIPTION),
        h('div', { className: 'dsh-ca-page', key: 'body' }, formBody(state, face)),
      ]);
    }
    ...
    function CompactAgentsCard(props) {
      ...
      // 正文与插件页共用同一份构建（见 formBody）：这里只负责把它塞进折叠区。
      const body = formBody(state, face);
    ...
    function apply(ctx) {
      ...
      // 三处都注册，谁有宿主谁渲染（`inject` 对未声明的槽位只是安静等待，见文件头）。
      // 同一 key/id 在同一槽位注册两次会抛，所以各自 try/catch：注册不上也绝不让其它处缺席。
      //   1. `settings.section`：设置侧栏里自成一页（与「内置插件」同级）—— 主要入口，且与宿主版本无关；
      //   2. `plugins.bundle.config`：新版插件页里，本包详情页的表单（键 = 包名）；
      //   3. `settings.plugin.item`：老版（≤ 0.1.5）设置页「插件 → 可配置」里的折叠卡片。
      registerSection(ctx, card);
      registerInto(ctx, 'plugins.bundle.config', PACKAGE_NAME, CompactAgentsPanel, card);
      registerInto(ctx, 'settings.plugin.item', NAMESPACE, CompactAgentsCard, card);
    }

    /**
     * 注册自成一页的设置分区（`settings.section` 是 **list** 槽位，形状与 keyed 槽位不同：
     * 用 `id` 认领位置、`order` 排位、`label` 给侧栏那一行文字，没有 `key`）。
     *
     * `order: 26` 紧挨着官方那几项（通用设置 0 / 模型 10 / 内置插件 15 / Agent 预设 20 /
     * 已归档会话 25），排在第三方插件（费用 30、插件市场 40…）之前。`label` 直接给字符串：
     * 本插件界面只有中文，`SlotLabel` 允许字符串（也可以是每次读取都重算的 thunk）。
     * @param ctx - 浏览器插件上下文。
     * @param card - 表单控制器。
     * @returns 无。
     */
    function registerSection(ctx, card) {
      try {
        ctx.slots.inject('settings.section', () => {
          try {
            return ctx.slots.register({
              ...
              label: TITLE,
              inject: () => card.inject(),
            }, CompactAgentsSection);
          } catch (error) {
            console.warn(`dsh-compact-agents: 注册设置分区失败：${error && error.message ? error.message : error}`);
            return () => {};
          }
        });
      } catch (error) {
        console.warn(`dsh-compact-agents: 订阅槽位 settings.section 失败：${error && error.message ? error.message : error}`);
      }
    }
     ...
     * @param ctx - 浏览器插件上下文。
     * @param slot - 槽位名。
     * @param key - 注册键（新页用包名，老设置页用 settings 命名空间）。
     * @param component - 该宿主要求的组件形态。
     * @param card - 表单控制器（两处共用同一个，保证两端状态一致）。
     * @returns 无。
     */
    function registerInto(ctx, slot, key, component, card) {
      try {
        ctx.slots.inject(slot, () => {
          try {
            return ctx.slots.register({ name: slot, key, inject: () => card.inject() }, component);
          } catch (error) {
            // 重复注册（同一 key 已在位）或槽位形态不符：记一行、放弃这一处，另一半照旧。
            console.warn(`dsh-compact-agents: 注册到 ${slot} 失败：${error && error.message ? error.message : error}`);
            return () => {};
          }
        });
      } catch (error) {
        console.warn(`dsh-compact-agents: 订阅槽位 ${slot} 失败：${error && error.message ? error.message : error}`);
      }
    }

    /** 需要的 cordis 服务：slots 提供表单槽位，settingsScope 提供命名空间读写通道。 */
