...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 两种生效时机 —— 字段按它分组渲染，每组的题注就是时机名 + 一句补充说明。
     * 于是「哪些改动立刻生效、哪些要等新建会话」只占两行小字，而不是每个字段各写一段；
     * 组内每个字段只有「标签 + 控件 + 一行 hint」。
     */
    const TIERS = [
      { id: 'live', name: '立即生效', note: '动作发生时读取，改完下一次压缩或续写即生效。' },
      { id: 'preset', name: '新建会话生效', note: '写入 preset 配置，只在新会话读取，已在运行的会话不受影响。' },
    ];

    /**
     * 卡片编辑的五个字段 —— 冻结的接口，字段名/类型/范围由宿主 half 的 settings 段决定。
     * kind 决定保存前的校验（'boolean' 只收 true/false，'int' 收整数，'number' 收数值），
     * tier 决定分组：live 两项宿主每次动作都读，preset 三项只在会话建立时读。
    ...
    const FIELDS = [
      ...
      },
      {
        ...
        tier: 'preset',
        label: '压缩触发阈值比例',
        hint: '上下文占用达到窗口的这个比例时触发压缩；默认 0.35（1M 窗口 ≈ 350K tokens）。',
      },
      {
        field: 'retainRatio',
        kind: 'number',
