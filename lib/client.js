/**
 * dsh-compact-agents 的浏览器 half：压缩参数的表单，挂到 DSH 的"插件配置"宿主上。
 *
 * ## 两个宿主，一份表单（DSH 0.1.6-alpha.2 起必须两个都注册）
 *
 ...
 * 删了），只注册它 = 卡片凭空消失、还没有任何报错。新宿主是 keyed 槽
 * `plugins.bundle.config`，**key 就是宿主 bundle 的包名**（新页按包名找表单：
 * `renderSlot('plugins.bundle.config', { view: 'page' }, { entryKey: pkg.name })`）。
 *
 * 所以这里**两个槽都注册**，谁有宿主谁渲染：
 *   - `plugins.bundle.config`（新页）：新页自己画标题、图标与面包屑，`view: 'page'`，
 *     组件只许渲染**表单主体**（字段 + 动作条），不能再套自己的卡片外壳；
 *   - `settings.plugin.item`（老设置页，≤ 0.1.5）：折叠卡片，头部、箭头、收起全由组件自己画。
 *
 * `ctx.slots.inject(name, cb)` 在槽位**尚未声明**时只是安静等待、声明后才跑 cb，因此
 * 对两个槽位都调用它是安全的（老版本永远等不到新槽，新版等不到旧槽），无需版本判断。
 * 每个 cb 里的 `register` 仍各自 try/catch：同一 key 重复注册会抛，一张表单注册不上也不该
 * 影响另一半。
 *
 ...
 *   3. 按 settings 命名空间注册表单：新页的 `plugins.bundle.config` 用**包名**当 key，
 *      老设置页的 `settings.plugin.item` 用**命名空间字符串** `compact-agents` 当 key
 *      （标签页据此把宿主 half 注册的命名空间与卡片配对）。两处都指向同一份表单与同一个
 *      `settingsScope` 绑定，所以参数与「已覆盖 / 重置」的语义两端完全一致。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // 两个槽位都注册，谁有宿主谁渲染（`inject` 对未声明的槽位只是安静等待，见文件头）。
      // 同一个 key 在同一槽位注册两次会抛，所以两处各自 try/catch：注册不上也绝不让另一半缺席。
